...
from model_v2 import OceanEmbedV2, DepthWeightedLoss
...
class OceanTempDataset(Dataset):
    """Dataset for GLORYS + surface inputs."""
    ...
    def __len__(self):
        return self.total_days * 3  # 3 random crops per day
...
def compute_metrics(pred, target, depths):
    """Compute RMSE, correlation, bias at each depth."""
...
def main():
    ...
    print(f"=== OceanEmbed V2 Training ({year}) ===")
    print(f"  Model: ViT-S v2 ({EMBED_DIM}d, {N_HEADS} heads, {N_LAYERS} layers)")
    print(f"  Loss: Depth-weighted MSE")
    print(f"  Optimizer: AdamW (lr={LR}, warmup={WARMUP_EPOCHS} epochs)")
    ...
    criterion = DepthWeightedLoss(n_depths=N_DEPTHS, depth_weight_power=0.5)
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            ...
            f"lr={get_lr(optimizer):.2e} | {dt:.1f}s"
        )
    ...
    print("\n--- Final Evaluation ---")
    checkpoint = torch.load(MODEL_DIR / "best_model_v2.pt", weights_only=False)
    ...
    metrics_path = LOG_DIR / f"metrics_v2_{year}.json"
    with open(metrics_path, "w") as f:
        json.dump(final_metrics, f, indent=2)
    print(f"\n  Metrics saved to {metrics_path}")

    history_path = LOG_DIR / f"history_v2_{year}.json"
    with open(history_path, "w") as f:
        json.dump(history, f, indent=2)

    print(f"\n=== DONE ===")
    print(f"  Best model: {MODEL_DIR / 'best_model_v2.pt'}")
